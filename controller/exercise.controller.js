...
class exerciseController {
  ...
  async getExercise(req, res) {
    const exercise = await db
      .query(
        `SELECT exercise.id, exercise.exercise_name, exercise.num_reps, exercise.time_reps, muscle_group.muscle_group, exercise_type.exercise_type, diff_lvl.diff_lvl, equipment_type.equipment_type, health_restriction.health_restriction
          FROM exercise
          ...
          JOIN health_restriction ON health_restriction.id = exercise.health_restriction_id;
      `
      )
      .catch((e) => {
        console.log(e);
  ...
  async updateExercise(req, res) {
    ...
    const exercise = await db
      .query(
        "UPDATE exercise set exercise_name=$1, id_muscle_group=$2, id_equipment_type=$3, id_diff_lvl=$4, id_exercise_type=$5, num_reps=$7, time_reps=$8 where id=$9 RETURNING *",
        [
          exercise_name,
          id_muscle_group,
          id_equipment_type,
